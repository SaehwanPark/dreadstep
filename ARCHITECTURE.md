...
- `dreadstep-core` owns `WorldState` transitions, occupancy, scheduling, combat, inventory,
  ...
  the stable state digest, including authored Kiter, Brute, and Frostcaster enemy intent preferences.
  The digest uses an explicit deterministic byte order, not a process-randomized hasher.
- Protocol v26 projects those values, including OpenDoor/Close terrain commands, actor behavior/status snapshots, Brute/Frostcaster enemy behavior, throwable item
