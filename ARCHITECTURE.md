...
The active Sprite-transform projection adds `PresentationBevySpriteTransformProjection`, an ordered
read-only join from each retained node to a map-space translation derived from checked
`ScenePixelPosition` metadata. Inventory entries remain unplaced, and this boundary does not attach
or mutate ECS Transform/Visibility/Sprite components; camera, window, renderer, and production media
remain deferred.
