...
delegates the mutation; neither boundary adds player pickup, effects, equipment, capacity, player
commands, or replay/history entries.
...
`SceneGroundItem` carries only the typed item identity, opaque definition reference, and position;
`sync_scene` keys those entities by globally unique `ItemId` and removes stale picked-up items. Bevy
does not own item data, effects, or pickup/drop rules, and this projection adds no rendering or
camera policy.
