...
- `dreadstep-core` owns `WorldState` transitions, occupancy, scheduling, combat, inventory,
  environmental tiles, canonical `RunOutcome`, replay traces, and the stable state digest.
  The digest uses an explicit deterministic byte order, not a process-randomized hasher.
- Protocol v18 projects those values. MCP, headless, and Bevy convert types and shape I/O;
