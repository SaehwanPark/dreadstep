...
- Active asset gate: local candidates are evaluated in the tracked
  [`asset-evaluation.md`](docs/presentation/asset-evaluation.md) record; 32×32 is the working tile
  target, the Kenney CC0 pack is a reusable pixel-art fallback, and UI audio is only a fallback.
  Dungeon combat/movement/item audio remains explicitly open.
- Next presentation work: a reversible headless-to-renderer spike using the selected placement and
  asset decisions, before widgets or text policy.
