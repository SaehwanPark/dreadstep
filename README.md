...
- Verified foundations
  ...
  - Deterministic adjacent closed-door interaction with typed `Door`, `Interact`, and `DoorOpened`
    values, atomic rejection, protocol v18/MCP/headless/Bevy mappings, desktop `I` control, and
  ...
  - Deterministic one-shot floor traps with walkable/non-blocking `Trap` terrain, shared `Move`/
    ...
    mutation, protocol v18/MCP/Bevy mappings, and smoke/journal coverage; discovery, disarming,
  ...
  - Deterministic adjacent breakable terrain with blocking `Breakable` tiles, scheduled `Break`,
    typed `BreakableBroken` evidence, atomic rejection, protocol v18/MCP/headless/Bevy mappings,
  ...
  - Deterministic kick-open door interaction with typed `Kick`, ordered `DoorOpened` plus fixed-radius
    `NoiseCreated` evidence, atomic rejection, protocol v18/MCP/headless/Bevy mappings, desktop `K`
    control, and smoke/journal coverage; noise propagation, enemy attraction, durability, and
    generic interaction dispatch remain deferred.
  ...
  - Fixed four-item inventory capacity with atomic full-inventory rejection across core, protocol
    v18 snapshots, MCP, headless command coverage, and Bevy legal-action projections; richer effects and
