...
use dreadstep_core::{
  Actor, ActorId, ActorKind, AmmunitionAmount, Command, Damage, EnemyBehavior as CoreEnemyBehavior,
  ...
  pub fn create_scenario(&mut self, scenario: &Scenario) -> Result<(), SessionError> {
    let tiles = scenario
      ...
      .map(|tile| match tile {
        ...
        ProtocolTile::OpenDoor => Tile::OpenDoor,
      ...
      .map(|actor| {
        ...
        Actor::with_melee_reach_and_behavior(
          ...
          match actor.behavior() {
            ...
            dreadstep_protocol::EnemyBehavior::Scavenger => CoreEnemyBehavior::Scavenger,
            dreadstep_protocol::EnemyBehavior::Zombie => CoreEnemyBehavior::Zombie,
          },
        )
      })
      .collect();
