//! MCP scenario projection for the Zombie archetype.
...
#[test]
fn scenario_preserves_zombie_and_projects_its_behavior() {
  ...
  session
    ...
    .expect("Zombie scenario should validate");

  assert_eq!(
    session
      .inspect(ActorId::new(2))
      .expect("Zombie should exist")
      .behavior(),
    EnemyBehavior::Zombie
  );
  ...
  let output = session.act(wait).expect("Zombie wait should succeed");
  assert_eq!(
    output.events(),
    &[Event::Waited {
      actor: ActorId::new(2),
