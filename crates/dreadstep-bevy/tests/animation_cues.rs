//! Deterministic headless animation-cue signal behavior.

use bevy::app::App;
use bevy::input::{ButtonInput, keyboard::KeyCode};
use dreadstep_bevy::{
  PresentationAnimationCue, PresentationAnimationCues, PresentationInput, PresentationPlugin,
  PresentationRuntime, PresentationState,
};
use dreadstep_core::{
  ActionTime, Actor, ActorId, ActorKind, BlockReason, Command, Damage, GridMap, HitPoints,
...
fn cues(app: &App) -> Vec<PresentationAnimationCue> {
  app
    .world()
    .resource::<PresentationAnimationCues>()
    .cues()
    .to_vec()
}

#[test]
fn fresh_runtime_has_no_animation_cues() {
  let mut app = cue_app(
...
#[test]
fn movement_and_blocking_preserve_typed_positions() {
  ...
  assert_eq!(
    cues(&moved),
    vec![PresentationAnimationCue::Moved {
      actor: ActorId::new(1),
  ...
  assert_eq!(
    cues(&blocked),
    vec![PresentationAnimationCue::MovementBlocked {
      actor: ActorId::new(1),
...
fn wait_preserves_action_time() {
  ...
  assert_eq!(
    cues(&app),
    vec![PresentationAnimationCue::Waited {
      actor: ActorId::new(1),
      at: ActionTime::new(0),
    }]
  );
...
fn attack_and_death_preserve_event_order_and_payloads() {
  ...
  assert_eq!(
    cues(&app),
    vec![
      PresentationAnimationCue::Attacked {
        attacker: ActorId::new(1),
        target: ActorId::new(2),
        damage: Damage::MELEE,
        remaining_hit_points: HitPoints::new(0),
      },
      PresentationAnimationCue::Died {
        actor: ActorId::new(2),
      },
    ]
  );
...
#[test]
fn missing_animation_cue_resource_is_a_safe_noop() {
  let mut app = App::new();
  ...
  assert!(
    app
      .world()
      .get_resource::<PresentationAnimationCues>()
      .is_none()
  );
}
