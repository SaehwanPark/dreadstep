...
- Candidate logical tile sizes: 24×24 and 32×32, both caller-configurable through
  `PresentationTileSize`; this record does not select between them because the generated PNGs
  below are unconstrained contact sheets rather than native-size samples.
...
| `kenney-tiny-dungeon-preview.png` | Terrain, structures, actors, and items | [Kenney Tiny Dungeon](https://kenney.nl/assets/tiny-dungeon); creator Kenney; Creative Commons CC0 | Unmodified official preview download, 918×515; SHA-256 `b6023d3a9504f84bb128d21680d0899c7a150bce3443d0861590f3f3de1c24b`; source page reports 16×16 tiles and CC0 | Cohesive reusable set with strong coverage and clear grid; the 16×16 source is a candidate for tested integer normalization, but no 24×24/32×32 normalized sample is retained yet; palette is brighter than the gothic target and needs normalization | Reusable fallback for a later prototype spike; production adoption remains pending normalization and attribution-record review |
...
1. **Tile size: unresolved.** The generated PNGs are 1254×1254 unconstrained contact sheets, not
   native 24×24 or 32×32 samples. Retain both caller-configurable options until a native-size or
   explicitly nearest-neighbor-normalized sample set is produced and measured.
2. **Pixel-art sourcing: mixed prototype path remains provisional.** Treat the generated sheets as
   visual direction and the CC0 Kenney pack as a reusable integration fallback. Neither is a final
   production art selection; any adopted binary requires a tracked provenance record and
   normalization pass.
...
The gate is complete for a renderer prototype when native or explicitly normalized tile samples
support a working-size decision, the mixed pixel-art fallback is consumed by a reversible
headless-to-renderer spike, and a target-specific audio set is selected or deliberately deferred
with an explicit production decision. Until then, no ignored binary is loaded by Bevy, referenced
by code, or treated as canonical game content.
