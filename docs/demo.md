...
| Opt-in procedural floor and `N` advancement | seeded 13×9 floor, two distinct inventory rarity/affix-tier choices, one bounded-potency consumable, two ground choices, depth-3+ Magic/Rare, magnitude-2, and potency-2 floors, and next-depth restart after victory | `run_started` depth and `floor_advanced` | no; smoke keeps item fixture |
