...
| Opt-in procedural floor and `N` advancement | seeded 13×9 floor, two inventory rarity/affix-tier choices, one bounded-potency consumable, two ground choices, and next-depth restart after victory | `run_started` depth and `floor_advanced` | no; smoke keeps item fixture |
